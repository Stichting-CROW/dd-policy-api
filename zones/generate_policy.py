...
def generate_policy(cur, data: zone.Zone):
    ...
    store_policy(cur, policy, data.geography_id)

def store_policy(cur, policy: mds.policy.Policy, geography_id):
    stmt = """
        INSERT INTO policies
        (policy_id, start_date, end_date, published_date, rules, gm_code, geography_ref, name, description)
        VALUES
        (%s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    cur.execute(stmt, (str(policy.policy_id), policy.start_date, policy.end_date, policy.published_date, 
        json.dumps(policy.rules, default=pydantic_encoder),  policy.municipality, str(geography_id), policy.name, policy.description))
